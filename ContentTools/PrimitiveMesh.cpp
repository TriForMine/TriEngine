...
namespace triengine::tools {
	namespace {
		...
		static_assert(_countof(creators) == primitive_mesh_type::count, "creators count mismatch");
		...
		mesh create_plane(const primitive_init_info& info, u32 horizontal_index = axis::x, u32 vertical_index = axis::z, bool flip_winding = false, v3 offset = { -0.5f, 0.f, -0.5f }, v2 u_range = { 0.1f, 0.1f }, v2 v_range = { 0.f, 1.f })
		{
			...
			assert(m.positions.size() == (horizontal_count + 1) * (vertical_count + 1));
