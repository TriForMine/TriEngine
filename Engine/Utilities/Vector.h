...
namespace triengine::utl {
	...
	public:
		...
		constexpr T* const erase(T *const item)
		{
			...
			{
				memmove(item, item + 1, (std::addressof(_data[_size]) - item) * sizeof(T));
			}
	...
	private:
		...
		constexpr void destruct_range(u64 start, u64 end)
		{
			assert(destruct);
			assert(start <= _size && end <= _size && start <= end);
			if (_data)
			{
				for (; start != end; ++start)
				{
