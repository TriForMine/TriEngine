...
namespace triengine::content {
	namespace {
		...
		bool read_transform(const u8*& data, game_entity::entity_info& info) {
			...
			XMFLOAT4A rot{ &rotation[0] };
			XMVECTOR quat{ XMQuaternionRotationRollPitchYawFromVector(XMLoadFloat4A(&rot)) };
			XMFLOAT4A rot_quat{};
		...
		component_reader component_readers[component_type::count]{ read_transform, read_script };
