...
namespace triengine::graphics::d3d12 {
	...
	void d3d12_surface::create_swap_chain(IDXGIFactory7* factory, ID3D12CommandQueue* cmd_queue, DXGI_FORMAT format /*= default_back_buffer_format*/)
	{
		...
		if (SUCCEEDED(factory->CheckFeatureSupport(DXGI_FEATURE_PRESENT_ALLOW_TEARING, &_allow_tearing, sizeof(_allow_tearing))) && _allow_tearing)
		{
	...
	void d3d12_surface::resize()
	{
		...
		DEBUG_OP(OutputDebugStringA("::D3D12 Surface Resized.\n"));
	}
	...
	void d3d12_surface::finalize()
	{
		...
		assert(width == _window.width() && height == _window.height());

		_viewport.TopLeftX = 0.0f;
		_viewport.TopLeftY = 0.0f;
		_viewport.Width = static_cast<f32>(width);
		_viewport.Height = static_cast<f32>(height);
