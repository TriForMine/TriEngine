...
namespace triengine::graphics::d3d12::d3dx {
	...
	ID3D12RootSignature* create_root_signature(const D3D12_ROOT_SIGNATURE_DESC1& desc)
	{
		...
		{
			DEBUG_OP(const char* error_msg{ error_blob ? (const char*)error_blob->GetBufferPointer() : "Unknown error" });
	...
	ID3D12Resource* create_buffer(const void* data, u32 buffer_size, bool is_cpu_accessible, D3D12_RESOURCE_STATES state, D3D12_RESOURCE_FLAGS flags, ID3D12Heap* heap, u64 heap_offset)
	{
		...
		if (data)
		{
			...
			// If we only want to upload some data to be used by the GPU, then is_cpu_accessible should be false
			if (is_cpu_accessible)
			{
				// NOTE: tange's Begin and ENd fields are set to 0, to indicate
